/**
 * Fly 官网共享侧边栏 v28 · 商业归因协议架构
 * 重构：从"信任系统"转为"收入系统"
 * 结构：AI Source → Evidence → Attribution → Settlement
 * 隐藏层：Trust/Verification/Governance（技术层不可见）
 */
(function(){
var sections=[
...
// ── 1. Revenue Flow（收入流）深绿 #1A3D2E ── AI归因经济链
{id:'identity',label:'Revenue Flow 收入流',color:'#1A3D2E',defaultOpen:true,icon:'<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 1v22m-4-4l4 4 4-4"/><path d="M5 12h14"/></svg>',
links:[
{href:'/concept-behavior-id.html',text:'AI Source AI来源',icon:'<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="3"/><path d="M12 1v2m0 18v2m-9-11h2m18 0h2m-3.3-7.7l-1.4 1.4M6.7 19.3l-1.4 1.4m0-15.4l1.4 1.4m10.6 12.6l1.4 1.4"/></svg>'},
{href:'/concept-action-id.html',text:'Evidence 证据',icon:'<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/></svg>'},
...
{href:'/concept-trust.html',text:'Attribution Ratio 归因比例',icon:'<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><polyline points="9 12 11 14 15 10"/></svg>'},
...
{href:'/agent-registry.html',text:'Agent Economy 经济生态',icon:'<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="3"/><path d="M12 1v2m0 18v2m-9-11h2m18 0h2m-3.3-7.7l-1.4 1.4M6.7 19.3l-1.4 1.4m0-15.4l1.4 1.4m10.6 12.6l1.4 1.4"/></svg>'},
...
{href:'/ai-skills.html',text:'Revenue Track 收入追踪',icon:'<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><polyline points="9 12 11 14 15 10"/></svg>'},
...
// ── 隐藏组 2-5：Trust/Verification/Attribution/Governance（技术层不可见）
];
// ── 构建侧边栏 ──
var nav=document.createElement('nav');nav.className='nav';
var navBody=document.createElement('div');navBody.className='nav-body';
sections.forEach(function(sec){
var secEl=document.createElement('div');secEl.className='nav-sec';
var btn=document.createElement('button');btn.className='nav-sec-btn';
btn.setAttribute('data-group',sec.id);
btn.style.setProperty('--sec-color',sec.color);
var iconSpan=document.createElement('span');iconSpan.className='nav-sec-icon';iconSpan.innerHTML=sec.icon;
btn.appendChild(iconSpan);
var labelSpan=document.createElement('span');labelSpan.className='nav-sec-label';labelSpan.textContent=sec.label;
btn.appendChild(labelSpan);
var arrow=document.createElement('span');arrow.className='nav-arrow';arrow.textContent='›';
btn.appendChild(arrow);
secEl.appendChild(btn);
var bd=document.createElement('div');bd.className='nav-sec-bd';
if(sec.defaultOpen)bd.classList.add('open');
if(sec.defaultOpen)btn.classList.add('arrow-open');
sec.links.forEach(function(link){
var a=document.createElement('a');a.className='nav-link';a.href=link.href;
a.setAttribute('data-group',sec.id);
a.innerHTML=link.icon+'<span>'+link.text+'</span>';
if(window.location.pathname===link.href)a.classList.add('active');
bd.appendChild(a);
});
secEl.appendChild(bd);
btn.addEventListener('click',function(){
bd.classList.toggle('open');
btn.classList.toggle('arrow-open');
});
navBody.appendChild(secEl);
});
