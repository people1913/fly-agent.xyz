/**
 * Fly API Worker v2.8.0 — Cloudflare Worker Entry Point
    ...
    const response = await (async () => {
      try {
        ...
        if (path === '/v1/health' && method === 'GET') {
          ...
          return json({ status, version: '2.8.0', layers: 8, protocols: 6, db: dbStatus, kv: kvStatus, timestamp: new Date().toISOString() });
        }
