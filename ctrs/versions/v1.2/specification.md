...
Agent Economy 正在成形。当多个自主 Agent 协作促成一笔商业交易——推荐 Agent 引流、讲解 Agent 促成决策、支付 Agent 完成交易——谁来证明谁贡献了多少？没有可验证的信任链，商业归因就会失真：贡献被低估的 Agent 得不到合理报酬，贡献被高估的 Agent 获得不当收益，Agent 经济的商业模式无法闭环。

CTRS 是为 Agent Economy 设计的**商业行为验证协议**。它解决的核心问题是 **Agent 经济中的商业归因失真**：

```
商业行为 → 证据 → 商业事实 → 归因 → 结算
```

CTRS 不是数据交换格式规范。它定义的是：如何将 Agent 的商业行为转化为可验证、可追溯、可审计的商业信任记录，使多 Agent 协作中的价值归因与结算具有机器可验证的确定性。

本规范定义了 **商业信任报告（Commercial Trust Report, CTRS）** 的数据格式、验证规则和交互协议，为 Agent 经济中的商业归因提供从行为到结算的完整信任链路。

### 1.2 范围

本规范适用于 Agent Economy 中的商业行为验证场景：

- 多 Agent 协作中的商业归因计算与验证
- Agent 商业行为的证据采集、Hash 绑定和防篡改验证
- 基于 Agent 贡献的结算分润场景
- 跨平台 Agent 商业协作的互操作验证
